...
static void _read_pointer(lv_libinput_t * dsc, struct libinput_event * event)
{
    ...
    switch(type) {
        ...
        case LIBINPUT_EVENT_TOUCH_DOWN: {
                lv_point_t point;
//                struct evdev_device *dev = evdev_device(touch_event->base.device);
//                LV_LOG_WARN("absinfo_x: min=%d max=%d resolution=%d",dev->abs.absinfo_x->minimum,dev->abs.absinfo_x->maximum,dev->abs.absinfo_x->resolution);
//                LV_LOG_WARN("absinfo_y: min=%d max=%d resolution=%d",dev->abs.absinfo_y->minimum,dev->abs.absinfo_y->maximum,dev->abs.absinfo_y->resolution);                
                double raw_x = libinput_event_touch_get_x(touch_event);
                double raw_y = libinput_event_touch_get_y(touch_event);
                LV_LOG_WARN("RAW: x=%.2f y=%.2f", raw_x, raw_y);
                double transformed_x = libinput_event_touch_get_x_transformed(touch_event, hor_res);
                double transformed_y = libinput_event_touch_get_y_transformed(touch_event, ver_res);
                LV_LOG_WARN("TRANSFORMED: x=%.2f y=%.2f (screen size: %d x %d)", transformed_x, transformed_y, hor_res, ver_res);
                point.x = (int32_t)LV_CLAMP(INT32_MIN, (int32_t)libinput_event_touch_get_x_transformed(touch_event, hor_res) - disp->offset_x,
                                            INT32_MAX);
                point.y = (int32_t)LV_CLAMP(INT32_MIN, (int32_t)libinput_event_touch_get_y_transformed(touch_event, ver_res) - disp->offset_y,
                                            INT32_MAX);
                LV_LOG_WARN("LVGL POINT: x=%d y=%d (offset: x=%d y=%d)",point.x, point.y, disp->offset_x, disp->offset_y);
